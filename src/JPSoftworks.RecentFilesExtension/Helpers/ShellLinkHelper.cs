...
/// </summary>
public sealed class ShellLinkHelper : IShellLinkHelper, IDisposable
{
    ...
    public string RetrieveTargetPath(string path)
    {
        ...
        // Get display name via SHCreateItemFromParsingName
        try
        {
            this.DisplayName = GetShellDisplayName(path);
        }
        catch
        {
    ...
    {
        // WinUI’s UI thread is already STA; this is just in case you call from a BG thread.
        //CoInitializeEx(COINIT.COINIT_APARTMENTTHREADED).ThrowOnFailure();

        unsafe
        {
            SHCreateItemFromParsingName(path, null!, typeof(IShellItem).GUID, out var item)
                .ThrowOnFailure();

            PWSTR pwsz;
            ((IShellItem)item).GetDisplayName(SIGDN.SIGDN_NORMALDISPLAY, &pwsz);
            var displayName = new string(pwsz.Value);
    ...
    private static unsafe (string IconPath, int IconIndex) GetFileIconLocation(string fileOrExtension)
    {
        ...
        {
            return (Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.System), "shell32.dll"), 1);
        }
        ...
        {
            return (Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.System), "shell32.dll"), 2);
        }
